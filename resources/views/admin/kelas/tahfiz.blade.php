          ...
          <select name="batch" class="form-control me-2">
            ...
            @foreach ($batches as $batch)
              <option value="{{ $batch }}" {{ $selectedBatch == $batch ? 'selected' : '' }}>
                {{ DB::table('tahfizs')->where('batch', $batch)->value('title') }} - Angkatan: {{ $batch }}
              </option>
            @endforeach
          </select>
    ...
    <div class="card mb-4">
      <div class="card-header d-flex justify-content-between mb-lg-n4">
        <div class="card-title">
          ...
          <small class="text-muted">Angkatan: {{ $batch }} @if(!empty(request('gelombang'))) | Gelombang: {{ $gelombang }} @endif</small>
          @endif
        </div>
        <div class="dropdown">
            ...
            <div class="border rounded p-3 mt-2">
              <div class="row gap-4 gap-sm-0">
                <div class="col-12">
                  ...
                  <a href="{{ route('export.kelas', ['programmableType' => $programmableType, 'batch' => $batch, 'gelombang' => $gelombang]) }}"
