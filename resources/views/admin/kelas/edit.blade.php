              ...
              <div class="col-sm-9">
                <select id="multicol-user" name="user_id" class="select2 form-select" data-allow-clear="true" required>
                  ...
                  @foreach ($users as $user)
                    <option value="{{ $user->id }}" {{ $user->id == $kelas->user_id ? 'selected' : '' }}>{{ $user->name }}</option>
                  @endforeach
                </select>
              </div>
            </div>
            ...
            <div class="row mb-5">
              ...
              <div class="col-sm-9">
                <select name="status" id="status" class="form-select" required>
                  <option value="Menunggu Update" @if ($kelas->status == 'Menunggu Update') selected @endif>Menunggu Update</option>
