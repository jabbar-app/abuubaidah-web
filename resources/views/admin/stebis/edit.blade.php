    ...
    <div class="d-flex justify-content-center">
      <div class="card col-xl-8 col-md-10 col-sm-12">
        ...
        <div class="card-body">
          ...
          <form action="{{ route('stebis.update', $stebis->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="mb-3">
              <label for="title" class="form-label">Nama Program</label>
              <input type="text" class="form-control" id="title" name="title" value="{{ $stebis->title }}">
            </div>
            <div class="mb-3">
              <label for="batch" class="form-label">Angkatan</label>
              <input type="number" class="form-control" id="batch" name="batch" value="{{ $stebis->batch }}">
            </div>
            ...
            <div class="mb-3">
              ...
              <textarea class="form-control" id="description" name="description">{{ $stebis->description }}</textarea>
            </div>

            <div class="mb-3">
              <label for="price_pra" class="form-label">Biaya Pendaftaran/Seleksi</label>
              <input type="number" class="form-control" id="price_pra" name="price_pra" value="{{ $stebis->price_pra }}">
            </div>
            <div class="mb-3">
            ...
            <div class="mb-3">
              ...
              <input type="number" class="form-control" id="price_mahad" name="price_mahad"
                value="{{ $stebis->price_mahad }}">
            </div>
            <div class="mb-3">
              <label for="price_s1" class="form-label">Biaya Pembangunan S1</label>
              <input type="number" class="form-control" id="price_s1" name="price_s1" value="{{ $stebis->price_s1 }}">
            </div>
            <div class="mb-3">
            ...
            <div class="mb-3">
              ...
              <input type="number" class="form-control" id="price_s1_nim" name="price_s1_nim"
                value="{{ $stebis->price_s1_nim }}">
            </div>
            <div class="mb-5">
              <label for="status" class="form-label">Status</label>
              <select name="status" id="status" class="form-select">
                <option value="1" @if ($stebis->status) selected @endif>Aktif</option>
                <option value="0" @if (!$stebis->status) selected @endif>Tidak Aktif</option>
              </select>
            </div>
