...
@section('content')
  <div class="container-xxl flex-grow-1 container-p-y">
    <h4 class="py-3 mb-4"><span class="text-muted fw-light">Program /</span>
      {{ $program->programmable->title ?? 'No Program Title' }}</h4>
        ...
        <div class="card mb-4">
          ...
          <form action="{{ route('create.invoice') }}" method="POST" class="card-body">
            ...
            @if (!empty($step->status) && $step->status == 'Daftar Ulang')
              <input type="hidden"
                value="{{ $program->programmable->price_normal + $program->programmable->price_mahad }}" name="amount">
            @else
            ...
            <div class="pt-4">
              <div class="row justify-content-end">
                <div class="col-sm-9">
                  ...
                  @else
                    <button type="submit"
                      class="btn btn-lg btn-primary">{{ $step->status ?? 'Lanjutkan Pendaftaran' }}</button>
                  @endif
                </div>
              </div>
            </div>
  ...
  <script type="text/javascript">
    $(document).ready(function() {
      ...
      function updatePricesWithNim(isRegistered) {
        ...
        var total = updatedPrices.price_pra + updatedPrices.price_normal + updatedPrices.price_mahad;
